...
from HomographyMatmaker import save_image
...
def generate_background(size=(image_size[0], image_size[1]), nb_blobs=100, min_rad_ratio=0.01,
...
def draw_checkerboard(img, max_rows=7, max_cols=7, transform_params=(0.05, 0.15)):
    ...
    thickness = random_state.randint(min_dim * 0.01, max(min_dim * 0.015, (min_dim * 0.01) + 1))
...
def draw_cube(img, min_size_ratio=0.2, min_angle_rot=math.pi / 10,
    ...
    for i in [0, 1, 2]:
        cv2.fillPoly(img, [cube[faces[i]].reshape((-1, 1, 2))],
                    col_face)
    thickness = max(random_state.randint(min_dim * 0.003, max(min_dim * 0.015, (min_dim * 0.003) + 1)), 1)
    for i in [0, 1, 2]:
        for j in [0, 1, 2, 3]:
...
draw_shape = [add_salt_and_pepper,
              draw_lines,
              draw_polygon,
...
# points = draw_shape[draw_shape_index](img)
# cv2.imshow("Shape drawn", img)
# img = draw_interest_points(img, points)
# cv2.imshow("With Points", img)
...
for i in range(5000):
    img = generate_background()
    points = None
    shape_no = np.random.randint(0, len(draw_shape))
    
    if shape_no != (len(draw_shape) - 1):
        points = draw_shape[shape_no](img)
    
    if np.random.uniform(0,1) > 0.5:
        final_blur(img)
    
    pointed_image = draw_interest_points(img, points)
    # cv2.imshow("img", pointed_image)
    # cv2.waitKey(100)
      
    if points is None:
        points = np.array([[]])
        
    save_image(img, 'Dataset/synthetic_shapes/images/output_' + str(i) + '.jpeg')
    with open('Dataset/synthetic_shapes/keypoints/output_' + str(i)+'.pkl', 'wb') as f:
        pickle.dump(points.tolist(), f)