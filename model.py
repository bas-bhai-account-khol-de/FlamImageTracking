...
def down_conv(inputs, filters):
    x = tf.keras.layers.MaxPooling2D()(inputs)
    x = tf.keras.layers.Conv2D(filters, (5,5), padding = "same", activation = "relu")(x)
    x = tf.keras.layers.Conv2D(filters, (5,5), padding = "same", activation = "relu")(x)
...
def get_model(input_shape, number_of_points):
    ...
    conv1 = tf.keras.layers.Conv2D(8, (5,5), padding = "same", activation = "relu")(inp)
    conv1 = tf.keras.layers.Conv2D(8, (5,5), padding = "same", activation = "relu")(conv1)
    ...
    upconv5 = tf.keras.layers.Conv2D(128, (5,5), padding = "same", activation = "relu")(conv4)
